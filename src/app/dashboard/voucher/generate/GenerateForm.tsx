...
export function GenerateForm({ 
  ...
  const autoSetLimitUptime = (profileName: string) => {
    const name = profileName.toLowerCase()
    if (name.includes('jam')) {
      const match = name.match(/\d+/)
      if (match) setLimitUptime(`${match[0]}h`)
    } else if (name.includes('hari')) {
      const match = name.match(/\d+/)
      if (match) setLimitUptime(`${match[0]}d`)
    } else if (name.includes('minggu')) {
      const match = name.match(/\d+/)
      if (match) setLimitUptime(`${parseInt(match[0]) * 7}d`)
    } else if (name.includes('bulan')) {
      setLimitUptime('30d')
    } else {
      setLimitUptime('')
    }
        ...
        if (result.error) {
          ...
          if (mode === "auto") {
            ...
            if (waNumber && waNumber.trim() !== "") {
              ...
              const selectedProfileObj = profiles.find(p => p.name.toLowerCase() === profile.toLowerCase());
              const price = selectedProfileObj ? selectedProfileObj.price : 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <select 
            name="profile"
            required
            onChange={(e) => autoSetLimitUptime(e.target.value)}
            className="w-full bg-[#0f172a] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all appearance-none"
          >
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-300 flex items-center justify-between">
            <span>Batas Waktu (Limit Uptime)</span>
            <span className="text-xs text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded-full">Penting!</span>
          </label>
          <input 
            type="text" 
            name="limitUptime"
            ...
            placeholder="Contoh: 1h, 12h, 3d, 30d (Kosongkan jika diatur di profil)"
          />
          <p className="text-xs text-slate-500 mt-1">Gunakan 'h' untuk jam, 'd' untuk hari. Jika profil belum mengunci waktu, Anda wajib mengisinya di sini.</p>
        </div>
